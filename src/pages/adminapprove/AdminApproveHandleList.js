...
function Approve() { // 관리자 사용 신청 내역 조회 페이지
  ...
  const SearchForm = (inputText) => { // 검색 기능
    ...
    axios({
      url: 'http://localhost:9191/admin/high/UserRequestHandleSearch',
      method: 'post',
      data: {
        inputText: inputText,
        pageNav : "관리자" + pageNav
  ...
  const kindBtn = (e) =>  { // 종류 버튼 (전체, 승인, 반려)
    ...
    axios({
      url: 'http://localhost:9191/admin/high/UserRequestNavSearch',
      method: 'post',
      data: {
        navText : "관리자" + navText
      },
  ...
  const assetsSelect = (e) =>  { // 자산별 조회 select
    if(e.target.value !== "0"){
      ...
      axios({
        url: 'http://localhost:9191/admin/high/UserRequestCategorySearch',
        method: 'post',
        data: {
  ...
  useEffect(() => { // 랜더링
    ...
    if(inputInnerData.username === "" || inputInnerData.length === 0){
      ...
      //axios.get('http://localhost:9191/admin/high/UserRequestHandlePage').then(res => setUserRequest(res.data));

      axios({
        url: "http://localhost:9191/admin/high/UserRequestHandlePage",
        method: "get",
        headers: {
          Authorization : token
        },
              ...
              <div className="col-lg-12">
                <div className="card">
                  <div className="card-body">
                    <h5 className="card-title">자산 최종 사용 처리내역 조회</h5>
