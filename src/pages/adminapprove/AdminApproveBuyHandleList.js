...
function AdminApproveBuyHandleList() {
  // 관리자 사용 신청 내역 조회 페이지
  ...
  const SearchForm = (inputText) => {
    ...
    axios({
      url: 'http://localhost:9191/admin/high/UserRequestBuyHandleSearch',
      method: 'post',
      data: {
        inputText: inputText,
        pageNav: '관리자' + pageNav,
  ...
  const kindBtn = (e) => {
    ...
    axios({
      url: 'http://localhost:9191/admin/high/UserRequestBuyNavSearch',
      method: 'post',
      data: {
        navText: '관리자' + navText,
      },
  ...
  const assetsSelect = (e) => {
    ...
    if (e.target.value !== '0') {
      ...
      axios({
        url: 'http://localhost:9191/admin/high/UserRequestBuyCategorySearch',
        method: 'post',
        data: {
...
export default AdminApproveBuyHandleList;
