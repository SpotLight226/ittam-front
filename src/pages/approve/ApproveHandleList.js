...
function Approve() { // 관리자 사용 신청 내역 조회 페이지
  ...
  const SearchForm = (inputText) => { // 검색 기능
    ...
    axios({
      url: 'http://localhost:9191/admin/UserRequestHandleSearch',
      method: 'post',
      data: {
        inputText: inputText,
        pageNav : "관리자" + pageNav
  ...
  const kindBtn = (e) =>  { // 종류 버튼 (전체, 승인, 반려)
    ...
    axios({
      url: 'http://localhost:9191/admin/UserRequestNavSearch',
      method: 'post',
      data: {
        navText : "관리자" + navText,
      },
  ...
  const assetsSelect = (e) =>  { // 자산별 조회 select
    if(e.target.value !== "0"){
      ...
      axios({
        url: 'http://localhost:9191/admin/UserRequestCategorySearch',
        method: 'post',
        data: {
  ...
  useEffect(() => { // 랜더링
    ...
    if(inputInnerData.username === "" || inputInnerData.length === 0){
      axios({
        url: "http://localhost:9191/admin/UserRequestHandlePage",
        method: "get",
        headers: {
          Authorization : token
        },
